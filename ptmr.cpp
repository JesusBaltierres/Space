...
{
    // Verificar si los tres caracteres son letras
    for (int i = 0; i < 3; i++)
    {
        if (!isalpha(cadena[i]))
        {
...
bool shots_collide(bool ship, int x, int y, int w, int h)
{
    for (int i = 0; i < SHOTS_N; i++)
    {
        ...
        // no chocar con mis propios disparos
...
int main()
{
    ...
    if (archivo == NULL)
    {
        ...
        fprintf(archivo, "1   AAA   0\n");
        fprintf(archivo, "2   AAA   0\n");
        fprintf(archivo, "3   AAA   0");
        fclose(archivo);
        ...
        while (!feof(archivo))
        {
            fscanf(archivo, "%d %s %ld", &v[i].pos, &v[i].nom, &v[i].puntos);
            printf("%d   %s   %ld\n", v[i].pos, v[i].nom, v[i].puntos);
            i++;
        }
        fclose(archivo);
    }
    ...
    if (score > v[0].puntos)
    {
        ...
        v[2].puntos = v[1].puntos;
        v[1].puntos = v[0].puntos;
        v[0].puntos = score;

        strcpy(v[2].nom, v[1].nom);
        strcpy(v[1].nom, v[0].nom);

        do {

            printf("Ingrese un Nombre con 3 letras: (Si escribe mas de 3 letras, solo se guardaran las primeras 3 letras ingresadas) \n");
            scanf("%3s", cadena); // Leer solo los primeros tres caracteres
        ...
        } while (validarCadena(cadena) != 1);


        strcpy(v[0].nom, cadena);
        ...
        for (i = 0; i < MAX; i++)
        {
            ...
            {
                fprintf(archivo, "%d   %s   %ld", v[i].pos, v[i].nom, v[i].puntos);
            }
        }
        fclose(archivo);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                fclose(archivo);
                printf("\nLa Puntuacion no entro al TOP 3 :( \n");
            }
        }
    }
